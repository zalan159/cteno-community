...
TASKS=00-management/tasks-cteno-agent-dialogue.json
LOG=/tmp/cteno-agent-dialogue-workflow.log
LOOPLOG=/tmp/cteno-agent-dialogue-loop.log
GUARDLOG=/tmp/cteno-agent-dialogue-guard.log
...
p = '$REPO/$TASKS'
try:
    with open('$REPO/00-management/.task-gate-state.cteno-agent-dialogue.json') as wf:
...
# from .task-gate-state.cteno-agent-dialogue.json), so this guard doesn't kill the auth
# workflow's node that is sharing the same binary path.
running = [i for i in d['items'] if i.get('status')=='running']
if running:
    my_pid = None
    try:
        with open('$REPO/00-management/.task-gate-state.cteno-agent-dialogue.json') as sf:
...
print(n)" 2>/dev/null)
  ...
  RESUME_ENV=""
  if [ -f "$REPO/00-management/.task-gate-state.cteno-agent-dialogue.json" ]; then
    RESUME_ENV="TASK_GATE_RESUME=1"
  fi
  env TASK_GATE_TASKS_PATH="$TASKS" \
      TASK_GATE_RUN_ID=cteno-agent-dialogue \
