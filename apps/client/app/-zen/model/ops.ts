 ...
 */
export async function fetchTodos(): Promise<TodoState> {
    // Fetch all KV items with todo prefix
    const response = await kvList({
        prefix: TODO_PREFIX,
 ...
 */
export async function initializeTodoSync(): Promise<void> {
    try {
        const todoState = await fetchTodos();
...
export async function addTodo(
    ...
    await todoLock.inLock(async () => {
        try {
            ...
            const indexResponse = await kvGet(TODO_INDEX_KEY);
            ...
            const result = await kvMutate(mutations);
        ...
        try {
            ...
            if (!todoResponse) {
                ...
                if (serverTodo.title !== title) {
                    ...
                    const newVersion = await kvSet(todoKey, encrypted, todoResponse.version);
