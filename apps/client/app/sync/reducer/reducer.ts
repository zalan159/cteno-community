...
import { Message, ToolCall, ImageAttachment } from "../typesMessage";
...
function mergeToolDeltaIntoInput(currentInput: any, patch: any): any {
...
function toolStartedAtFromElapsed(createdAt: number, patch: any): number | null {
    const elapsedValue =
        patch && typeof patch === 'object' && !Array.isArray(patch)
            ? patch.elapsedTimeSeconds
            : undefined;
    if (typeof elapsedValue !== 'number' || !Number.isFinite(elapsedValue) || elapsedValue < 0) {
        return null;
    }
    return Math.max(0, createdAt - elapsedValue * 1000);
}
...
export function reducer(state: ReducerState, messages: NormalizedMessage[], agentState?: AgentState | null): ReducerResult {
    ...
    for (let msg of nonSidechainMessages) {
        if (msg.role === 'user') {
            ...
            let mid = allocateId();
            // Extract text and images from content (supports both single text and array formats)
            let userText: string;
            let userImages: ImageAttachment[] | undefined;
            if (Array.isArray(msg.content)) {
                userText = msg.content
                    .filter((b): b is { type: 'text'; text: string } => b.type === 'text')
                    .map(b => b.text)
                    .join('');
                const imgBlocks = msg.content.filter((b: any) => b.type === 'image');
                if (imgBlocks.length > 0) {
                    userImages = imgBlocks.map((b: any) => {
                        if (b.source.type === 'file_ref') {
                            return { media_type: b.source.media_type, file_id: b.source.file_id };
                        }
                        return { media_type: b.source.media_type, data: b.source.data };
                    });
                }
                console.log(`[REDUCER] User message ${msg.id}: array content with ${msg.content.length} blocks, ${imgBlocks.length} images extracted`);
            } else {
            ...
            // Process text, thinking, and image content (tool calls handled in Phase 2)
            let lastAgentTextMid: string | null = null;
            for (let c of msg.content) {
                if (c.type === 'text' || c.type === 'thinking') {
                    ...
                    state.messages.set(mid, {
                        ...
                        role: 'agent',
                        createdAt: msg.createdAt,
                        // Render thinking blocks with dedicated UI styling (not markdown italics)
                    ...
                    changed.add(mid);
                    if (!isThinking) lastAgentTextMid = mid;
                } else if (c.type === 'image') {
                    // Build ImageAttachment from image content block
                    const img: any = {};
                    const source = (c as any).source;
                    if (source) {
                        img.media_type = source.media_type;
                        img.data = source.data;
                    } else if ((c as any).image_url) {
                        img.media_type = 'image/png';
                        img.data = undefined;
                        img.url = (c as any).image_url;
                    }

                    // Attach to preceding text message, or create standalone
                    if (lastAgentTextMid !== null) {
                        const prev = state.messages.get(lastAgentTextMid);
                        if (prev) {
                            const images = prev.images ? [...prev.images, img] : [img];
                            state.messages.set(lastAgentTextMid, { ...prev, images });
                            changed.add(lastAgentTextMid);
                        }
                    } else {
                        let mid = allocateId();
                        state.messages.set(mid, {
                ...
                if (c.type === 'tool-call') {
                    ...
                    if (existingMessageId) {
                        ...
                        if (message?.tool) {
                            ...
                            // Update createdAt to match the ACP message's server timestamp.
                            // Permission messages created by Phase 0 (from agentState) arrive before
                            // ACP messages (which go through DB storage), so they have a browser timestamp
                            // that may be out of order relative to thinking messages. Using the ACP
                            // message's server timestamp ensures correct ordering with other ACP messages.
                            ...
                            if (c.input && typeof c.input === 'object' && !Array.isArray(c.input)) {
                                const nextInput = message.tool.permission
                                    ? { ...c.input, ...(message.tool.input ?? {}) }
                                    : { ...(message.tool.input ?? {}), ...c.input };
                                if (Object.prototype.hasOwnProperty.call(c.input, 'terminalInteraction')) {
                                    nextInput.terminalInteraction = c.input.terminalInteraction;
                                }
                                message.tool.input = nextInput;
                            }
    ...
    //
    // Phase 2.5: Process non-sidechain tool deltas
    //
    ...
    for (let msg of nonSidechainMessages) {
        ...
        for (let c of msg.content) {
            ...
            const messageId = state.toolIdToMessageId.get(c.id);
            if (!messageId) {
                continue;
            }
            ...
            message.tool.input = mergeToolDeltaIntoInput(message.tool.input, c.patch);
            const startedAt = toolStartedAtFromElapsed(msg.createdAt, c.patch);
            if (startedAt !== null) {
