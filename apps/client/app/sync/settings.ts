...
export const AIBackendProfileSchema = z.object({
    ...
    defaultPermissionMode: z.enum(['default', 'auto', 'acceptEdits', 'plan', 'dontAsk', 'bypassPermissions', 'read-only', 'safe-yolo', 'yolo']).optional(),
...
export const SettingsSchema = z.object({
    ...
    compactActiveSessions: z.boolean().describe('Whether to use compact view for active sessions'),
    ...
    lastUsedRuntimeProfileId: z.string().nullable().describe('Last selected runtime model profile ID for new sessions'),
    lastUsedRuntimeEffort: z.string().nullable().describe('Last selected runtime reasoning effort for new sessions'),
...
export const settingsDefaults: Settings = {
    ...
    lastUsedRuntimeProfileId: null,
    lastUsedRuntimeEffort: null,
...
export function settingsParse(settings: unknown): Settings {
    ...
    const legacyCompactSessionView = (settings as { compactSessionView?: unknown }).compactSessionView;
    if (
        parsed.data.compactActiveSessions === undefined
        && typeof legacyCompactSessionView === 'boolean'
    ) {
        parsed.data.compactActiveSessions = legacyCompactSessionView;
    }
