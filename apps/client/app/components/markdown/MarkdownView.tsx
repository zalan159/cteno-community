...
const TRUSTED_DOWNLOAD_HOSTS = getTrustedDownloadHosts();
...
export const MarkdownView = React.memo((props: { 
    ...
    onLocalFilePress?: (path: string) => void;
}) => {
                    ...
                    if (block.type === 'text') {
                        return (
                            <RenderTextBlock
                                ...
                                onLocalFilePress={props.onLocalFilePress}
                            />
                        );
...
function RenderTextBlock(props: {
    ...
    // Check if any span contains a cteno-file:// link
    const ctenoFileLink = props.spans.find(span => span.url && span.url.startsWith('cteno-file://'));
    if (ctenoFileLink) {
        const fileId = ctenoFileLink.url!.replace('cteno-file://', '');
        const filename = ctenoFileLink.text;
        return <FileDownloadCard fileId={fileId} filename={filename} />;
    }

    // Check if this text block is a single download link
    if (props.spans.length === 1 && props.spans[0].url) {
        const url = props.spans[0].url;
        const filename = props.spans[0].text;

        // Check for regular download link
        if (isDownloadLink(url)) {
            return <FileDownloadCard url={url} filename={filename} />;
        }

        if (props.onLocalFilePress) {
            const localPathFromLink = normalizeLocalFilePathCandidate(url);
            if (localPathFromLink) {
                return <LocalFileCard path={localPathFromLink} onPress={props.onLocalFilePress} />;
            }
        }
    }

    if (props.onLocalFilePress) {
        const localPath = detectLocalFilePath(props.spans);
        if (localPath) {
            return <LocalFileCard path={localPath} onPress={props.onLocalFilePress} />;
        }
    }
...
function detectLocalFilePath(spans: MarkdownSpan[]): string | null {
    if (!spans.length || spans.some((s) => !!s.url)) return null;
    const raw = spans.map((s) => s.text).join('').trim();
    return normalizeLocalFilePathCandidate(raw);
}

function normalizeLocalFilePathCandidate(rawValue: string): string | null {
    ...
    if (candidate.startsWith('~')) {
        // Keep "~" for caller-side expansion using session homeDir.
        candidate = `~${candidate.slice(1)}`;
    }

    // Strip optional line/column suffix: /path/to/file.ts:12 or :12:3
    candidate = candidate.replace(/:(\d+)(?::(\d+))?$/, '');
    // Strip optional #Lline / #LlineCcol suffix.
    candidate = candidate.replace(/#L\d+(?:C\d+)?$/i, '');
...
function LocalFileCard(props: { path: string; onPress: (path: string) => void }) {
    const fileName = React.useMemo(() => {
        const normalized = props.path.replace(/\/+$/, '');
        const idx = normalized.lastIndexOf('/');
        if (idx >= 0) return normalized.slice(idx + 1) || normalized;
        return normalized;
    }, [props.path]);
    ...
    return (
        <Pressable
            ...
            style={({ pressed }) => [
                style.localFileCard,
                pressed && style.localFileCardPressed,
            ]}
        >
            <View style={style.localFileCardHeader}>
                <FileIcon fileName={fileName} size={22} />
                <View style={style.localFileCardTextWrap}>
                    <Text numberOfLines={1} style={style.localFileCardTitle}>{fileName}</Text>
                    <Text numberOfLines={1} style={style.localFileCardPath}>{props.path}</Text>
                </View>
...
// Helper function to detect download links
function isDownloadLink(url: string): boolean {
    try {
        const urlObj = new URL(url);
        // Check if it's from known file hosting domains
        if (TRUSTED_DOWNLOAD_HOSTS.some((host) => urlObj.hostname.includes(host))) return true;
        if (urlObj.hostname.includes('aliyuncs.com')) return true;

        // Check if URL path looks like a file (has file extension)
        const pathname = urlObj.pathname.toLowerCase();
        const hasFileExtension = /\.(pdf|zip|rar|7z|tar|gz|jpg|jpeg|png|gif|webp|svg|mp4|mov|avi|mkv|webm|mp3|wav|flac|m4a|aac|txt|md|json|xml|csv|doc|docx|xls|xlsx|ppt|pptx)$/.test(pathname);
