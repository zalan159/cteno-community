...
export const QuotaPopover = React.memo(({ machineId, vendor, preferredModelId }: Props) => {
    ...
    // the only registered machine. Quota is machine-scoped but in practice
    ...
    const quota = useVendorQuota(effectiveMachineId, vendor);
    ...
    const primary = pickPrimaryQuota(quota, preferredModelId);
    ...
    const triggerLabel = (() => {
        if (quota?.error) return `• 额度: 不可用`;
        if (!primary) return `• 额度: …`;
...
function QuotaDetails({ quota }: { quota: VendorQuota | null }) {
    const styles = stylesheet;
    const { theme } = useUnistyles();

    if (!quota) {
        return <Text style={styles.planMeta}>正在获取额度…</Text>;
    }
    if (quota.error) {
        return <Text style={styles.errorRow}>{quota.error}</Text>;
    }

    const rows: React.ReactNode[] = [];
    if (quota.shape === 'windows') {
        rows.push(
            <Text key="title" style={styles.sectionTitle}>{vendorTitle(quota.provider)}</Text>,
        );
        const order = ['fiveHour', 'weekly', 'weeklyOpus', 'weeklySonnet', 'overage'];
        const keysInOrder = [
            ...order.filter((k) => quota.windows?.[k]),
            ...Object.keys(quota.windows ?? {}).filter((k) => !order.includes(k)),
        ];
        if (keysInOrder.length === 0) {
            rows.push(<Text key="empty" style={styles.planMeta}>暂无窗口数据</Text>);
        }
        for (const key of keysInOrder) {
            const w = quota.windows?.[key];
            if (!w) continue;
            rows.push(
        ...
        rows.push(
            <Text key="title" style={styles.sectionTitle}>{vendorTitle(quota.provider)} · 模型限额</Text>,
        );
    ...
    if (quota.planType) {
        rows.push(
            <Text key="plan" style={styles.planMeta}>
                计划：{quota.planType}
                {quota.credits?.balance ? `  ·  余额 ${quota.credits.balance}` : ''}
            </Text>,
        );
    }
