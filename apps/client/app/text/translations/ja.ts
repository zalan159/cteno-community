/**
 * Japanese translations for the Cteno app
 * Values can be:
...
export const ja: TranslationStructure = withEnglishFallback({
    tabs: {
    ...
    profiles: {
        ...
        baseURL: 'ベースURL',
        authToken: '認証トークン',
        authTokenGlobalHint: 'APIキー（空欄 = グローバルキーを使用）',
        ...
        chatCompressSummary: ({ chatModel, compressModel }: { chatModel: string; compressModel: string }) =>
            `チャット: ${chatModel} | 圧縮: ${compressModel}`,
        ...
        noMachineConnected: '接続されたマシンがありません',
        storageHint: 'LLMプロファイルはマシン側に保存されます。セッションごとに別のプロファイルを使えます。',
        failedToSave: 'プロファイルの保存に失敗しました',
        failedToDelete: 'プロファイルの削除に失敗しました',
        cannotDelete: 'プロファイルを削除できません',
    ...
    connect: {
        ...
        restoreQrStep1: '1. モバイル端末で Cteno を開く',
        restoreQrStep2: '2. 設定 -> アカウント に移動',
        restoreQrStep3: '3. 「新しいデバイスをリンク」をタップ',
        restoreQrStep4: '4. このQRコードをスキャン',
    },
    ...
    settings: {
        ...
        about: 'このアプリについて',
        aboutFooter: 'Cteno はあなたの個人AIアシスタントです。完全なエンドツーエンド暗号化を採用し、アカウントはデバイスにのみ保存されます。',
        whatsNew: '新機能',
    ...
    settingsAppearance: {
        ...
        compactActiveSessions: 'コンパクトセッション表示',
        compactActiveSessionsDescription: 'アクティブなセッションをコンパクトなレイアウトで表示',
    },
    ...
    newSession: {
        ...
        title: '新しいセッションを開始',
        noMachinesFound: 'マシンが見つかりません。まずコンピューターでCteno セッションを起動してください。',
        ...
        profileBuiltIn: '組み込み',
        profileRequiresCliOnly: ({ required }: { required: string }) => `このプロファイルは ${required} CLI のみを使用します`,
        profileCliNotDetected: ({ cliName }: { cliName: string }) => `${cliName} CLI が検出されません（このプロファイルで必要です）`,
        wizardProfileTitle: 'AIプロファイルを選択',
        wizardProfileDescription: 'セッションで使用するAIバックエンドを選択します。別API用のカスタムプロファイルを作成できます。',
        wizardMachineTitle: 'マシンを選択',
        wizardDirectoryTitle: '作業ディレクトリを選択',
        wizardPermissionTitle: '権限モード',
        wizardAdvancedOptions: '詳細オプション',
        wizardLlmProfileTitle: 'LLMプロファイル（ネイティブエージェント）',
        wizardLlmProfileDescription: 'マシンのネイティブエージェントが使用するLLM設定を選択します。プロファイルは設定で管理できます。',
        cliNotDetected: ({ cliName }: { cliName: string }) => `${cliName} CLI が検出されません`,
        dismissPopupFor: 'このポップアップを今後表示しない対象',
        thisMachine: 'このマシン',
        anyMachine: 'すべてのマシン',
        installPrefix: 'インストール',
        viewInstallationGuide: 'インストールガイドを見る →',
        installGeminiCli: '利用可能なら Gemini CLI をインストール',
        viewGeminiDocs: 'Gemini ドキュメントを見る →',
        duplicate: '複製',
        searchMachinesPlaceholder: '入力してマシンを絞り込み...',
        recentMachinesTitle: '最近のマシン',
        favoriteMachinesTitle: 'お気に入りのマシン',
        noMachinesAvailable: '利用可能なマシンがありません',
        searchDirectoriesPlaceholder: '入力して絞り込み、またはカスタムディレクトリを入力...',
        recentDirectoriesTitle: '最近のディレクトリ',
        favoriteDirectoriesTitle: 'お気に入りのディレクトリ',
        noRecentDirectories: '最近のディレクトリがありません',
        permissionDesc: {
            askForPermissions: '権限を確認する',
            readOnlyMode: '読み取り専用モード',
            workspaceWriteWithApproval: '承認付きでワークスペース書き込み',
            fullAccessSkipPermissions: 'フルアクセスで権限確認を省略',
            autoApproveEdits: '編集を自動承認',
            planBeforeExecuting: '実行前に計画',
            skipAllPermissions: 'すべての権限確認を省略',
        },
        sessionType: {
            title: 'セッションタイプ',
            simple: 'シンプル',
    ...
    server: {
        ...
        notValidHappyServer: '有効なサーバーではありません',
    ...
    sessionInfo: {
        ...
        updateCliInstructions: '最新バージョンに更新してください',
        ...
        switchedToProfile: ({ name }: { name: string }) => `プロファイルを ${name} に切り替えました`,
        failedToSwitchProfile: 'プロファイルの切り替えに失敗しました',
        switchLlmProfileTitle: 'LLMプロファイルを切り替え',
        switchLlmProfileMessage: 'このセッションのプロファイルを選択:',
        llmProfile: 'LLMプロファイル',
        fullSessionObject: 'セッションオブジェクト全文',
    ...
    components: {
        emptyMainScreen: {
            ...
            readyToCode: 'AIアシスタントを始めますか？',
            installCli: '新しいセッションを開始',
            runIt: '実行する',
    ...
    machineLauncher: {
        ...
        removeFavoriteMessage: ({ item, section }: { item: string; section: string }) =>
            `${section} から「${item}」を削除しますか？`,
        removeFavoriteConfirm: '削除',
    },
    ...
    settingsAccount: {
        ...
        deleteAccountConfirm: 'アカウントと会話履歴、設定、決済履歴を含むすべてのデータが完全に削除されます。この操作は取り消せません。',
        deleteAccountFinalTitle: '本当に削除しますか？',
        deleteAccountFinalConfirm: 'アカウントとすべてのデータが完全に削除され、元に戻すことはできません。',
        deleteAccountFinalButton: '完全に削除',
    },
    ...
    settingsScheduledTasks: {
        title: 'スケジュールタスク',
        subtitle: '時刻指定タスクと定期タスクを管理',
        noMachineAvailable: '利用可能なマシンがありません',
        emptyTitle: 'スケジュールタスクはありません',
        emptySubtitle: '会話で Cteno に「何を」「いつ」行うかを指示できます',
        details: '詳細',
        enable: '有効化',
        disable: '無効化',
        enabled: '有効',
        ...
        disabled: '無効',
        oneTime: '一回限り',
        errors: ({ count }: { count: number }) => `エラー: ${count}`,
        ok: '成功',
        fail: '失敗',
        runCount: ({ count }: { count: number }) => `${count} 回実行`,
        scheduleOnce: ({ time }: { time: string }) => `一回: ${time}`,
        everySeconds: ({ count }: { count: number }) => `${count}秒ごと`,
        everyMinutes: ({ count }: { count: number }) => `${count}分ごと`,
        everyHours: ({ count }: { count: number }) => `${count}時間ごと`,
        everyDays: ({ count }: { count: number }) => `${count}日ごと`,
        scheduleCron: ({ expr }: { expr: string }) => `Cron: ${expr}`,
        overdue: '期限超過',
        nextUnderMinute: '次回: 1分未満',
        nextMinutes: ({ count }: { count: number }) => `次回: ${count}分`,
        nextHours: ({ count }: { count: number }) => `次回: ${count}時間`,
        nextAt: ({ time }: { time: string }) => `次回: ${time}`,
        minutesAgo: ({ count }: { count: number }) => `${count}分前`,
        hoursAgo: ({ count }: { count: number }) => `${count}時間前`,
        daysAgo: ({ count }: { count: number }) => `${count}日前`,
        editTask: 'タスクを編集',
        ...
        yesAutoDelete: 'はい',
        noRecurring: 'いいえ',
        scheduleOneTime: ({ time }: { time: string }) => `一回限り: ${time}`,
        seconds: ({ count }: { count: number }) => `${count} 秒`,
        minutes: ({ count }: { count: number }) => `${count} 分`,
        hours: ({ count }: { count: number }) => `${count} 時間`,
        days: ({ count }: { count: number }) => `${count} 日`,
        interval: '間隔',
        unitMin: '分',
        unitHr: '時',
        unitDay: '日',
        runAtIso: '実行時刻（ISO 8601）',
        ...
        scheduleRecurring: ({ interval, anchor }: { interval: string; anchor: string }) =>
            `繰り返し: ${interval}ごと${anchor ? `（基準: ${anchor}）` : ''}`,
        saving: '保存中...',
        edit: '編集',
        confirm: '確認しますか？',
        close: '閉じる',
    ...
    terminal: {
        ...
        terminalRequestDescription: 'ターミナルがCtenoアカウントへの接続を要求しています。これにより、ターミナルは安全にメッセージを送受信できるようになります。',
    ...
    oauth: {
        connectService: ({ service }: { service: string }) => `${service} を接続`,
        runCommandInTerminal: '次のコマンドをターミナルで実行してください:',
    },
    ...
    welcome: {
        ...
        scanToJoinAccount: 'スキャンしてアカウントに参加',
        unsupportedQRCode: 'サポートされていないQRコードです。他のデバイスのアカウントQRコードをスキャンしてください。',
