...
//!      into our per-model `UsageBucket` shape (`usedPercent = (1 -
...
use crate::store::{UsageBucket, VendorId, VendorUsage};
...
#[async_trait]
impl VendorUsageProbe for GeminiProbe {
    ...
    async fn poll(&self) -> Result<VendorUsage, String> {
        ...
        let creds = match load_oauth_creds(&creds_path) {
            Ok(c) => c,
            Err(e) => return Ok(VendorUsage::error(VendorId::Gemini, e)),
        };
...
fn translate_quota(response: &Value) -> VendorUsage {
    let mut out = VendorUsage::new_buckets(VendorId::Gemini);
