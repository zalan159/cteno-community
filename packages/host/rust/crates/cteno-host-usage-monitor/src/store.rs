//! Vendor-neutral usage snapshot types.
//!
//! The store holds one `VendorUsage` per known vendor. Frontend reads the
...
/// One per-model request/token bucket (Gemini). Like `UsageWindow` but
...
pub struct UsageBucket {
    ...
    /// 0–100，统一成"已用"，跟 UsageWindow.used_percent 对齐。
    pub used_percent: f64,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub resets_at: Option<i64>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl VendorUsage {
    pub fn new_windows(provider: VendorId) -> Self {
        Self {
            provider,
            shape: VendorUsageShape::Windows,
            windows: HashMap::new(),
            buckets: Vec::new(),
            credits: None,
            plan_type: None,
    ...
    pub fn new_buckets(provider: VendorId) -> Self {
        Self {
            provider,
            shape: VendorUsageShape::Buckets,
            windows: HashMap::new(),
            buckets: Vec::new(),
            credits: None,
            plan_type: None,
    ...
    pub fn error(provider: VendorId, message: impl Into<String>) -> Self {
        ...
        let shape = match provider {
            VendorId::Claude | VendorId::Codex => VendorUsageShape::Windows,
            VendorId::Gemini => VendorUsageShape::Buckets,
        };
        Self {
            provider,
            shape,
...
pub struct VendorUsageMap {
    #[serde(flatten)]
    pub entries: HashMap<String, VendorUsage>,
}

impl VendorUsageMap {
    pub fn insert(&mut self, vendor: VendorId, usage: VendorUsage) {
        self.entries.insert(vendor.as_str().to_string(), usage);
    }

    pub fn get(&self, vendor: VendorId) -> Option<&VendorUsage> {
