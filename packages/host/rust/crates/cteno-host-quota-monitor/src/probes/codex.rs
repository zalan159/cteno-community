...
//!   4. Translate the `rateLimitsByLimitId` map into our `QuotaWindow` shape.
//!
...
use crate::store::{QuotaCredits, QuotaWindow, VendorId, VendorQuota};
...
#[async_trait]
impl VendorQuotaProbe for CodexProbe {
...
fn parse_credits(value: &Value) -> Option<QuotaCredits> {
    Some(QuotaCredits {
        has_credits: value
            .get("hasCredits")
...
fn translate_snapshot(response: &Value) -> VendorQuota {
    let mut out = VendorQuota::new_windows(VendorId::Codex);
...
mod tests {
    ...
    fn translate_rate_limits_by_limit_id() {
        ...
        let quota = translate_snapshot(&response);
        assert_eq!(quota.plan_type.as_deref(), Some("pro"));
        assert!(quota.error.is_none());
        let five = quota.windows.get("fiveHour").expect("fiveHour window");
        assert_eq!(five.used_percent as i64, 2);
        assert_eq!(five.resets_at, Some(1_776_711_831));
        let weekly = quota.windows.get("weekly").expect("weekly window");
        assert_eq!(weekly.used_percent as i64, 33);
        let credits = quota.credits.clone().expect("credits");
    ...
    fn translate_missing_payload_sets_error() {
        let quota = translate_snapshot(&json!({}));
