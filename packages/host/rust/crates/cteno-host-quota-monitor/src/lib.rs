//! Machine-level quota/rate-limit monitor for external agent vendors.
//!
...
pub use probe::VendorQuotaProbe;
pub use store::{
    QuotaBucket, QuotaCredits, QuotaWindow, VendorId, VendorQuota, VendorQuotaMap, VendorQuotaShape,
};
...
pub struct QuotaMonitor {
    store: Arc<RwLock<VendorQuotaMap>>,
    tx: broadcast::Sender<VendorQuota>,
}
...
impl QuotaMonitor {
    pub fn new() -> Arc<Self> {
        ...
        Arc::new(Self {
            store: Arc::new(RwLock::new(VendorQuotaMap::default())),
            tx,
        })
    }

    /// Read a cloned snapshot of the current cache.
    pub async fn snapshot(&self) -> VendorQuotaMap {
        self.store.read().await.clone()
    }

    /// Subscribe to live updates. Each successful or errored probe tick
    /// emits one `VendorQuota` value on this channel.
    pub fn subscribe(&self) -> broadcast::Receiver<VendorQuota> {
        self.tx.subscribe()
    }
    ...
    where
        P: VendorQuotaProbe + Send + Sync + 'static,
    {
                ...
                let snapshot = match probe.poll().await {
                    Ok(s) => {
                        ...
                        if let Some(err) = &s.error {
                            log::info!(
                                "[quota-monitor] {} probe reported user-visible error: {}",
                                vendor.as_str(),
                                err
                            );
                        } else {
                            log::info!(
                                "[quota-monitor] {} probe ok: windows={} buckets={} plan={:?}",
                                vendor.as_str(),
                                s.windows.len(),
                                s.buckets.len(),
                                s.plan_type,
                    ...
                    Err(e) => {
                        ...
                        log::warn!(
                            "[quota-monitor] {} probe error ({} in a row): {}",
                            vendor.as_str(),
                            consecutive_errors,
                            e
                        );
                        VendorQuota::error(vendor, e.to_string())
                    }
                };
    ...
    pub async fn set(&self, quota: VendorQuota) {
        let vendor = quota.provider;
        let mut guard = self.store.write().await;
        guard.insert(vendor, quota.clone());
        drop(guard);
        let _ = self.tx.send(quota);
    }
}
