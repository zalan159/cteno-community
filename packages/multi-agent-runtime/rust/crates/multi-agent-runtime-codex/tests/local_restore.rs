...
use multi_agent_protocol::{
    ...
    create_codex_workspace_profile, create_coding_studio_template, instantiate_workspace,
};
use multi_agent_runtime_codex::{
    CodexApprovalPolicy, CodexSandboxMode, CodexWorkspace, CodexWorkspaceOptions,
};
...
fn restores_and_deletes_local_workspace() {
    ...
    assert_eq!(
        workspace.runtime().snapshot().session_id.as_deref(),
        Some("codex-root-thread")
    );
