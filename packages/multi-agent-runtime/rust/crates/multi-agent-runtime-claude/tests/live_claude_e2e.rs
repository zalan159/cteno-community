...
use multi_agent_protocol::{
    ...
    WorkspaceTurnRequest, create_autoresearch_template, create_claude_workspace_profile,
...
fn live_tests_enabled() -> bool {
    std::env::var("CTENO_LIVE_CLAUDE").ok().as_deref() == Some("1")
}

#[tokio::test]
async fn claude_live_workspace_turn_coding_delegates_to_prd() {
    if !live_tests_enabled() {
        eprintln!("[skip] set CTENO_LIVE_CLAUDE=1 to run live tests");
        return;
    }
...
async fn claude_live_workspace_turn_opc_delegates_to_finance() {
    ...
    let mut workspace =
        build_workspace("claude-live-opc", &cwd, create_opc_solo_company_template());
    let startup_events = workspace.start();
    assert!(startup_events.len() >= 2);
